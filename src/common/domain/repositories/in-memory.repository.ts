...
export type CreatePropos = {
  [key: string]: any
}

export abstract class InMemoryRepository<Model extends ModelProps> implements RepositoryInterface<Model, CreatePropos> {
  items: Model[] = [];
  sortableFields: string[] = [];

  create(props: CreatePropos): Model {
    const model = {
      id: randomUUID(),
      create_at: new Date(),
      updated_at: new Date(),
